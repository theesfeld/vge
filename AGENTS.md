...
- **Live glass:** `./cmfd.sh` or `cargo run --release --bin cmfd` — **vehicle systems only** (jet formats remain in `src/jet/` for widgets reuse, not product path). **No SIM** — empty glass until OBD LIVE.
